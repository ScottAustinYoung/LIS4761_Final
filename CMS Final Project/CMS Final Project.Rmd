...
```{r, "Load Data"}
# Load the filtered data ONCE after cleaning and take care of problems of FIPS code error
filtered_data <- read_csv("Data/filtered_data.csv",
                          col_types = cols(FIPS_Code = col_character())) %>%
  mutate(FIPS_Code = as.numeric(gsub("[^0-9]", "", FIPS_Code)))
...
#tigirs enabled for kmeans web element
