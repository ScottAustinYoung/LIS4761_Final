...
```{r, "Step 2"}
### Model 1: Grouping by NPI compared against HCPCS codes
...
write.csv(df_Apriori_NPI_HCPCS_Code, "df_Apriori_NPI_HCPCS_Code.csv", row.names = FALSE)
```
...
# Choose specialty
selected_speciality <- "Internal Medicine"

# Filter and summarize
hm_filtered_data <- medicare_cleaned %>%
  filter(Provider_Type == selected_speciality) %>%
...
tampa_plot <- ggplot(data = tampa_map_data) +
  ...
  labs(
    ...
    subtitle = paste("Speciality:", selected_speciality),
  ) +
